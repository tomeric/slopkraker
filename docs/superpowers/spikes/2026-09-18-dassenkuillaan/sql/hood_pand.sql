WITH hood AS (SELECT geom FROM neighbourhoods WHERE name = 'Geleen-Noord'),
parts AS (
  ...
  FROM buildings b, hood
  WHERE b.source = 'bag3d' AND ST_Intersects(ST_Centroid(b.geom), hood.geom)
),
