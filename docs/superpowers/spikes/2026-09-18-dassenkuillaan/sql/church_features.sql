WITH hood AS (SELECT ST_Buffer(ST_SetSRID(ST_MakePoint(186330, 332234), 28992), 4000) AS geom),
parts AS (
  ...
  FROM buildings b
  WHERE b.source = 'bag3d' AND split_part(b.source_id, '/', 1) IN (
    SELECT DISTINCT split_part(x.source_id, '/', 1) FROM buildings x JOIN buildings o ON o.source = 'osm' AND o.kind IN ('church', 'chapel') AND ST_Intersects(x.geom, o.geom) AND ST_Area(ST_Intersection(x.geom, o.geom)) > 20, hood
    WHERE x.source = 'bag3d' AND ST_Intersects(o.geom, hood.geom))
),
