...
--
-- Takes :cx :cy :radius, the same island window window.sql takes, so the clusters and the
-- parts they name come out of the same set of buildings.
...
parts AS (
  ...
  FROM buildings b, win WHERE b.source = 'bag3d' AND ST_DWithin(b.geom, win.c, :radius)
),
mains AS (
